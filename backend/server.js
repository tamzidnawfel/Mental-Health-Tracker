...
app.get('/api/patient-home/accessibility-analysis', (req, res) => {
    ...
    const patientCheckSql = 'SELECT patient_id FROM PATIENT WHERE patient_id = ?';
    db.query(patientCheckSql, [patientId], (patientErr, patientRows) => {
        if (patientErr) {
        ...
        const appointmentsSql = `
            ...
            JOIN PATIENT p ON a.patient_id = p.patient_id
            JOIN PROVIDER pr ON a.provider_id = pr.provider_id
            LEFT JOIN REGION region ON p.district_id = region.district_id
            WHERE a.status = 'Confirmed'
            ORDER BY region.district_name ASC, p.name ASC, a.appointment_id ASC
        `;

        db.query(appointmentsSql, (appointmentsErr, appointmentRows) => {
            if (appointmentsErr) {
                console.error('Appointment accessibility error:', appointmentsErr);
                return res.status(500).json({ error: 'Failed to load accessibility analysis.' });
            }
            ...
            db.query(allProvidersSql, (providerErr, providerRows) => {
                if (providerErr) {
                    console.error('Provider lookup error:', providerErr);
                    return res.status(500).json({ error: 'Failed to load providers for accessibility analysis.' });
                }
                ...
                const summary = {
                    total_patients: new Set(patients.map(patient => patient.patient_id)).size,
                    financial_barriers: new Set(
                        patients
                            .filter(patient => ['High Financial Barrier', 'Moderate Financial Barrier'].includes(patient.financial_barrier))
